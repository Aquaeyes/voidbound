...
object VoidBoundMemoryTypeRegistry {
    ...
    var MEMORY_TYPES: LazyRegistrar<MemoryModuleType<*>> = LazyRegistrar.create(BuiltInRegistries.MEMORY_MODULE_TYPE, VoidBound.modid)
    ...
    var INPUT_STORAGE_LOCATIONS: RegistryObject<MemoryModuleType<MutableList<BlockPos>>> = MEMORY_TYPES.register("input_storage_locations") {
        MemoryModuleType<MutableList<BlockPos>>(Optional.empty())
    }

    var OUTPUT_STORAGE_LOCATION: RegistryObject<MemoryModuleType<BlockPos>> = MEMORY_TYPES.register("output_storage_location") {
        MemoryModuleType<BlockPos>(Optional.empty())
    }

    var OUTPUT_STORAGE_LOCATIONS: RegistryObject<MemoryModuleType<MutableList<BlockPos>>> = MEMORY_TYPES.register("output_storage_locations") {
        MemoryModuleType<MutableList<BlockPos>>(Optional.empty())
    }
}
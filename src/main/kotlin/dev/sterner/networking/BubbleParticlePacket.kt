...
class BubbleParticlePacket(val vector3f: Vector3f) : LodestoneClientPacket() {
    ...
    override fun executeClient(
        ...
        if (client.level != null) {
            val scaleData = GenericParticleData.create(0.1f, RandomHelper.randomBetween(client.level!!.random, 1.7f, 1.8f), 0.5f)
                .setEasing(Easing.SINE_OUT, Easing.SINE_IN).setCoefficient(RandomHelper.randomBetween(client.level!!.random, 1f, 1.25f))
                .build()

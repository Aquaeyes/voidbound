...
class SummonAllyGoal(private val cleric: CrimsonClericEntity) : Goal() {
    ...
    private fun summonAlly(cleric: CrimsonClericEntity) {
        val cultist = if (cleric.random.nextBoolean()) {
            VoidBoundEntityTypeRegistry.CRIMSON_ARCHER_ENTITY.get().create(cleric.level())
        } else {
            VoidBoundEntityTypeRegistry.CRIMSON_KNIGHT_ENTITY.get().create(cleric.level())
        }

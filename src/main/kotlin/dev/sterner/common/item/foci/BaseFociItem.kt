...
class BaseFociItem(val foci: IWandFocus, val type: MalumSpiritType, properties: Properties) : Item(properties), IVoidItem {
    ...
    override fun spawnEarlyParticles(
        ...
        if (foci is WardingFoci) {
            spawnLightItemScreenParticles(target, level, voidParticleIntensity, partialTick)
        } else {
            ScreenParticleEffects.spawnVoidItemScreenParticles(target, level, voidParticleIntensity, partialTick)
        }
    }
